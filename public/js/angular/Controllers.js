...
.controller('lession2',['$scope','Service','$timeout',function($scope,Service,$timeout){
	...
	angular.extend($scope,{
		...
		restart: function(){
			$scope.edit('Results',$scope.results.R1,0,$scope.results.R3,$scope.results.R4,$scope.results.user_id,$scope.results.id_result);
			$scope.getAll('Examples');
			$scope.hits = 0;
			$scope.dynamic = $scope.hits;
			Service.msg($scope);
...
.controller('lession3',['$scope','Service','$timeout',function($scope,Service,$timeout){
	...
	angular.extend($scope,{
					...
					if ($scope.hits == 10) {
						...
						if ($scope.hits == 10) {
							...
							if ($scope.results.R3 == null) {
								...
								$scope.edit('Results',$scope.results.R1,$scope.results.R2,$scope.hits/2,$scope.results.R4,$scope.results.user_id,$scope.results.id_result,$scope.saveTrue);
							}
						}else{
							$scope.success($timeout, Service);
						}
		...
		restart: function(){
			$scope.edit('Results',$scope.results.R1,$scope.results.R3,0,$scope.results.R4,$scope.results.user_id,$scope.results.id_result);
			$scope.getAll('Examples');
			$scope.hits = 0;
			$scope.dynamic = $scope.hits;
			Service.msg($scope);
...
.controller('lession4',['$scope','Service','$timeout',function($scope,Service,$timeout){
	...
	angular.extend($scope,{
					...
					if ($scope.hits == 10) {
						...
						if ($scope.hits == 10) {
							...
							if ($scope.results.R4 == null) {
								...
								$scope.edit('Results',$scope.results.R1,$scope.results.R2,$scope.results.R3,$scope.hits/2,$scope.results.user_id,$scope.results.id_result,$scope.saveTrue);
							}
						}else{
							$scope.success($timeout, Service);
						}
		...
		restart: function(){
			$scope.edit('Results',$scope.results.R1,$scope.results.R2,$scope.results.R3,0,$scope.results.user_id,$scope.results.id_result);
			$scope.getAll('Examples');
			$scope.hits = 0;
			$scope.dynamic = $scope.hits;
			Service.msg($scope);
